...
# Create asymmetry dataframes for each subject for peripheral or foveal or 1st and 2nd pass 
...
for (ID in IDs){
  ...
  subjectdf = dftrials[which(dftrials$ID == ID & dftrials$trialnumber<=162),] # Select the data you need for each subject
...
subjectdf1 <- subjectdf[which(subjectdf$trialnumber<=162 & subjectdf$Ecc=='PF'),]
subjectdf2 <- subjectdf[which(subjectdf$trialnumber<=162 & subjectdf$Ecc=='FF'),]
subjectdf3 <- subjectdf[which(subjectdf$trialnumber>=163 & subjectdf$Ecc=='PF'),]
subjectdf4 <- subjectdf[which(subjectdf$trialnumber>=163 & subjectdf$Ecc=='FF'),]
